...
# do learning here
uncertainty_dist = GaussianUniformError(0.01, 1.0)

discretization = UniformDiscretization(DiscreteState([-1.0, -1.0], [1.0, 1.0]), [0.5, 0.5])
abstraction = transition_intervals(discretization, linear_system_mat, process_noise_dist, uncertainty_dist)
...
for _ in 1:refinement_steps
    ...
    abstraction = refine_abstraction(abstraction, linear_system_mat, discretization.compact_space, amb_states, process_noise_dist, uncertainty_dist)
    amb_states = verify_and_plot(abstraction, spec_filename)
end
